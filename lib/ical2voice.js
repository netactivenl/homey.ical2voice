...
function triggerNextEventPerCalendar() {
    ...
    if (calendars) {
        calendars.forEach(function(calendar) {
            ...
            if (nextEvent) {
                app.log("Next event found for calendar " + calendar.name + ": " + nextEvent.summary + " " + nextEvent.start.format("YYYY-MM-DD") + "@" + nextEvent.start.format("HH:mm"));
            ...
            for (var k in data) {
                if (data.hasOwnProperty(k)) {
                    ...
                    if (event.type === "VEVENT" && event.start) {
                        var occurrences;
                        if (event.rrule) {
                            // Recurring event, determine any relevant occurrences.
...
function getRelevantEventOccurrences(event) {
    ...
    if (event.summary === "Herhaling 3 dagen") {
        app.log("Determining next recurrence for event '" + event.summary + "'...");
    }

    var count = 0;
    var maxCount = event.rrule.options.count;
    ...
    if (event.rrule.options.freq === 2) {
        ...
        if (event.rrule.options.byweekday) {
        ...
        var nextWeeklyRecurrence = firstWeeklyOccurrence;
        ...
        while (nextWeeklyRecurrence.isSameOrBefore(lastAcceptedOccurrence) && count < dow.length) {
            if (dow.indexOf(nextWeeklyRecurrence.day()) > -1) {
                count++;
                if (maxCount) {
                    //app.log(event.summary + " " + count + "/" + maxCount + ": " + moment(nextWeeklyRecurrence).format());
                }
                if (nextWeeklyRecurrence.isSameOrAfter(now)) {
    ...
    if (event.rrule.options.freq === 3) {
        var firstDailyOccurrence = moment(event.start);

        var dailyOccurrences = [];
        var nextDailyRecurrence = firstDailyOccurrence;
        nextDailyRecurrence.hour(firstDailyOccurrence.hour());
        nextDailyRecurrence.minute(firstDailyOccurrence.minute());
        nextDailyRecurrence.second(firstDailyOccurrence.second());

        while (nextDailyRecurrence.isSameOrBefore(lastAcceptedOccurrence) && count < maxCount) {
            count++;
            if (maxCount) {
                //app.log(event.summary + " " + count + "/" + maxCount + ": " + moment(nextDailyRecurrence).format());
            }
            if (nextDailyRecurrence.isSameOrAfter(now)) {
                dailyOccurrences.push(moment(nextDailyRecurrence).utc());
            }
    ...
    // DONE: Add support for larger recurrence intervals (now 1 is assumed for each event).
    // DONE: Add support for specific number of recurrences.
...
function updateSettings(settings, callback) {
    // Update settings.
    app.log("Updating settings...");
    calendars = settings.calendars;
    refreshIntervalInMinutes = parseInt(settings.refreshIntervalInMinutes);
    Homey.ManagerSettings.set("calendars", calendars);
    Homey.ManagerSettings.set("calendar_refresh_interval", refreshIntervalInMinutes);
    ...
    // Return success
    app.log("Updating settings completed.");
