...
module.exports = {
    init: function(appRef) {
        ...
        log("Loaded " + calendars.length + " calendar(s) from settings.");

        // Listen for flow triggers.
        // TODO: Homey.manager("flow").on("next_appointment_in", next_appointment_in_trigger);
        // TODO: Homey.manager("flow").on("action.todays_schedule", todays_schedule_action);
        // TODO: Homey.manager("flow").on("action.todays_remaining_schedule", todays_remaining_schedule_action);
        // TODO: Homey.manager("flow").on("action.next_appointment", next_appointment_action);
        // TODO: Homey.manager("flow").on("action.tomorrows_schedule", tomorrows_schedule_action);
        // TODO: Homey.manager("flow").on("action.tomorrows_first_appointment", tomorrows_first_appointment_action);
        // TODO: Homey.manager("flow").on("action.weekday_schedule", dow_schedule_action);
        ...
        calendars.forEach(function(calendar) {
            updateCalendarEvents(calendar, function(error, events) {
                if (error) {
                    ...
                    log("Calendar '" + calendar.name + "' updated, " + events.length + " event(s) found. Next update in " + refreshIntervalInMinutes + " minute(s).");
                }
            });
        });
    }
...
function triggerNextEventPerCalendar() {
    log("Triggering next event per calendar...");
    if (calendars) {
        calendars.forEach(function(calendar) {
            var nextEvent = getNextEvent(calendar.events);
            log("Next event found for calendar " + calendar.name + ": " + nextEvent);
            if (nextEvent) {
                var tokens = {
...
function next_appointment_in_trigger(callback, args, state) {
    log("trigger.next_appointment_in");
    ...
    if (args.timespan_number !== "" && args.timespan_unit !== "" && state) {
        ...
        if (isSameMoment) {
            log("trigger.next_appointment_in continued.");
        }
...
function todays_schedule_action(callback, args) {
    log("action.todays_schedule");
...
function todays_remaining_schedule_action(callback, args) {
    log("action.todays_remaining_schedule");
...
function next_appointment_action(callback, args) {
    log("action.next_appointment");
...
function tomorrows_schedule_action(callback, args) {
    log("action.tomorrows_schedule");
...
function tomorrows_first_appointment_action(callback, args) {
    log("action.tomorrows_first_appointment");
...
function dow_schedule_action(callback, args) {
    log("action.dow_schedule: " + args.dow);
...
function updateCalendarEvents(calendar, callback) {
    ical.fromURL(calendar.url, {}, function(error, data) {
        ...
        if (error) {
                ...
                if (data.hasOwnProperty(k)) {
                    ...
                    if (event.type === "VEVENT" && event.start) {
                        ...
                        if (event.rrule) {
                            ...
                            //if (occurrences)
                            //    log("Found " + occurrences.length + " occurrences for '" + event.summary + "' in the next year.");
                            //else
                            //    log("Found no occurrences for '" + event.summary + "' in the next year.");
                        } else {
                            // Single occurrence.
                        ...
                        if (occurrences) {
                            for (var i = 0; i < occurrences.length; i++) {
                                (function (e, start) {
                                    //log(JSON.stringify(start) + " isSameOrAfter " + JSON.stringify(today));
            ...
            //log(calendar.events.length + " events retrieved from ical: " + calendar.url);
...
function getRelevantEventOccurrences(event) {
    ...
    if (event.rrule.options.until !== null && moment.utc(event.rrule.options.until).isBefore(now)) {
        //log("No more reccurrences for event '" + event.summary + "' (ended: " + moment(event.rrule.options.until).format() + ").");
        return null;
    }

    //log("Determining next recurrence for event '" + event.summary + "'...");
    ...
    if (event.rrule.options.freq === 2) {
        //if (event.summary === "Raymond: VRIJ") log(JSON.stringify(event));
        ...
        //log("valid dow: " + JSON.stringify(dow));
    ...
    log("No recurrence found for event '" + event.summary + " (start: " + moment.utc(event.start).format() + "): " + JSON.stringify(event.rrule.options));
...
function log(message) {
    app.log(moment().format("HH:mm:ss.SSS") + " - " + message);
}